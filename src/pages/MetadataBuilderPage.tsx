...
const MetadataBuilderPage: React.FC = () => {
  ...
  const convertToOpenApi = async () => {
    ...
    try {
      ...
      } else if (parsedData.baseUrl || parsedData.host) {
        openApiDoc.servers = [{
          url: parsedData.baseUrl || `https://${parsedData.host}`,
          description: 'Auto-detected server'
        }];
      }

      // Auto-detect paths/endpoints
      if (parsedData.paths) {
        openApiDoc.paths = parsedData.paths;
      } else if (parsedData.endpoints || parsedData.resources) {
        const endpoints = parsedData.endpoints || parsedData.resources;
        endpoints.forEach((endpoint: any) => {
          const path = endpoint.path || endpoint.url || endpoint.uri || '/';
